...
"  random =>variables and =>entropy terms are entered.\n"
...
// simple style
"The form of random variables and variable lists depend on the =>style.\n"
...
// full style
"The form of random varirables and variable lists depend on the =>style.\n"
...
"are build from a list of random =>variables. The notation depends on\n"
"the =>style. In simple (lazy) style with separating character %1$c these\n"
...
// full style
"Basic entropy terms such as conditional entropy or mutual information\n"
"are build from a list of ramdom =>variables. The notation depends on\n"
"the =>style. In full style it follows the standard one:\n"
...
"is enclosed in parantheses. Arguments are separated by either %c or |.\n"
...
"    macro add T(a|b|c;d) = -H(a|b,c) + 2(a;c|b,d)-7*[a;b;c;d]\n");
        printf(
"Only variables in the argument list can be used in the right hand size\n"
...
"or a variable list; the separators much match those in the definition. Thus\n");
       if(minitip_style==syntax_short) printf(
...
/** EXPRESSION **/
    if(strncasecmp(argv,"expr",3)==0){ // linear combination
        printf(
"An expression is a linear combination of =>entropy terms and\n"
"=>macro invocations, such as\n");
        if(minitip_style==syntax_short) printf(
"      -1.234*I(x|y) - 12.345(a%1$cb|h)+X(x%1$cb|aY)\nwhere (X%1$c|)",minitip_sepchar);
        else printf(
"       -1.234*I(X|Y) - 12.345I(a;b|H)+X(X;b|a,Y)\nwhere X(;|)");
        printf(
" is a macro. The * sign between the constant and the entropy\n"
"term is optional and can be omitted.\n");
        return 0;
    }
/** RELATION **/
...
"       I(X;Y1|Z,Y2)+I(X;Y2|Z)=I(X;Y1,Y2|Z)\n");
        printf(
"Only this three comparison operators can be used. Any side (but not\n"
...
"        zap I(A;B|C)+I(B;C|A)+I(C;A|B) =='\n");
        printf(
"Similarly to the 'check' keyword, 'zap' can also be omitted.\n");
        return 0;
    }
...
"  entropy    -- entropy term syntax and shorthands\n"
...
{int i;
    if(*line==0 || *line=='?' || strcmp(line,"help")==0){ // empty line, help
        ...
        "*  a =>relation comparing two entropy expressions;\n"
...
{int i0,i1,pos; int not_printed;
    if(*arg=='?' || strcmp(arg,"help")==0){ // help
        ...
        printf(" Use 'list all' to show all; 'list 1-10' to show the first 10 constraints.\n"
...
"the style to SIMPLE (lazy). Enter 'syntax style' for a more help.\n");
    return 0; /* OK */
}
...
"separators; they define different macros.\n");
         }
         return 0;
    }
...
{"lp-iterlimit",	100000,	100,	100000000,	"LP iteration limit"},
{"lp-timelimit",	10,	1,	10000,		"LP time limit in seconds"},
{NULL,			0,	0,	0,		NULL}
};
...
{PARAMETERS *P; int n; char buf[50];
    ...
    if(!line) printf("List / set runtime parameters. Use \"set\" without arguments\n"
    "to list changeable parameters; use \"set parameter-name=value\" to change the\n"
...
{char *ret; char *constr;
    ...
    ret=call_lp(compute_expression,get_param("lp-iterlimit"),get_param("lp-timelimit"));
    if(ret==EXPR_TRUE){
        printf("      ==> TRUE%s\n",constr);
