...
#define e_TOO_MANY_MACRO "cannot add more macros, the maximum of " mkstringof(minitip_MAX_MACRONO) " has been reached"
...
static struct macro_text_t macro_text[minitip_MAX_MACRONO];
...
{int i; struct entropy_expr_t *e;
    ...
    if(macro_total<minitip_MAX_MACRONO) macro_total++;
}
...
{char sep; int done,v,var,defpos;
 struct macro_head_t head;
    ...
    must(v<0,v<4?e_MDEF_NOSTD:e_MDEF_DEFINED);
                           // we must have space
    if(macro_total >= minitip_MAX_MACRONO-1) softerr(e_TOO_MANY_MACRO);
    if(syntax_error.softerrstr|| syntax_error.harderrstr) return 1;
    clear_entexpr();       // clear the result space
    no_new_id(NULL);       // add new variables
    id_table_idx=0;        // don't keep old identifiers
